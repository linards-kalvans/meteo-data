...
def raw_lv_grid(config: MDConfig) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    forecast_data = pl.concat([data[0].with_columns(pl.lit(config.grid[i]["iso_3166-2"]).alias("iso_3166_2")) for i, data in enumerate(all_data)])
    current_data = pl.concat([data[1].with_columns(pl.lit(config.grid[i]["iso_3166-2"]).alias("iso_3166_2")) for i, data in enumerate(all_data)])
